...
#    print J_ob
    ...
    H = tau_pre - np.dot(winv_pre, np.dot(P_pre, np.dot(aux10_pre, qdd_pre) + h_pre))
    tde_u = np.dot(winv, np.dot(P, np.dot(aux10, qddot_des) + h)) + H
...
#                      np.dot(qr_Q.T, Mqh_bar_des - np.dot(S.T, invdyn))))
    ...
    return np.dot(S.T, tde_u) + kp * (q_des - q) + kd * (qdot_des - dq), q_des, qdot_des
